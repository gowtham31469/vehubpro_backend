...
> **Stack**: React.js (Frontend) · Django REST Framework (Backend) · PostgreSQL (Database) · AWS S3 (Media)  
...
STACK:
- Frontend: React.js 18 with React Router v6, Redux Toolkit, Axios, TailwindCSS, shadcn/ui
...
│   │   │   └── public/          # Public portfolio pages (no auth)
│   │   ├── components/
...
│   │   ├── utils/               # Helpers (FY calc, GST, date)
│   │   └── constants/           # Enums, roles, config
│   └── public/
│
├── backend/                     # Django project
│   ├── config/                  # Django settings (base, dev, prod)
│   ├── apps/
│   │   ├── core/                # Tenant, User, Subscription models
│   │   ├── services_module/     # Customer, Vehicle, ServiceMaster, JobCard, Invoice
│   │   ├── portfolio_module/    # InventoryVehicle, TestDriveBooling
│   │   ├── notifications/       # Email, SMS, WhatsApp services
│   │   ├── storage/             # S3 upload/download service
│   │   └── audit/               # AuditLog model + middleware
│   ├── utils/
│   │   ├── encryption.py        # AES-256-GCM encrypt/decrypt + HMAC blind index
│   │   ├── gst.py               # GST calculation engine
│   │   ├── invoice_number.py    # FY-based invoice sequence generator
│   │   ├── s3.py                # Presigned URL generator, upload helpers
...
│   └── nginx.conf
│
└── .cursorrules                  # Cursor AI rules (above)
```
...
### TASK 1.7 — React Project Bootstrap

**Cursor Prompt:**
```
Create a React 18 project with Vite at frontend/ with:
- react-router-dom v6, @reduxjs/toolkit, react-redux, axios, tailwindcss, @shadcn/ui
- react-hook-form + yup for form validation
- recharts for analytics charts
- react-hot-toast for notifications
- dayjs for date handling

Folder structure: src/apps/super-admin/, src/apps/tenant/, src/apps/public/

Create src/services/api.js:
- Axios instance with baseURL from env VITE_API_URL
- Request interceptor: attach Authorization: Bearer {access_token} from Redux store
- Response interceptor: on 401 → attempt refresh token via POST /api/auth/token/refresh/, on failure → dispatch logout() and redirect /login
- Response interceptor: unwrap { success, data, meta } envelope — return data directly

Create src/store/authSlice.js:
- State: { user, accessToken, refreshToken, isLoading }
- Thunks: loginThunk (POST /api/auth/login/), logoutThunk, refreshThunk
- Persist tokens in httpOnly cookie via backend Set-Cookie, NOT localStorage (XSS protection)

Create src/utils/fy.js:
- getCurrentFY() -> "25-26" string
- getFYLabel(fyCode) -> "FY 2025-26" for display

Create src/components/ui/ base components: PageHeader, DataTable (with sort, filter, pagination), StatusBadge, ConfirmModal, FormField (wraps react-hook-form Controller with Yup error display).
```
...
## 🚀 PHASE 7 — Deployment (Sprint 12)

### TASK 7.1 — Docker + Production Setup

**Cursor Prompt:**
```
Create Docker and deployment configuration:

infra/docker-compose.yml services: db (postgres:15), redis, backend (Django), celery-worker, celery-beat, frontend (nginx serving React build), nginx-proxy

infra/Dockerfile.backend:
- FROM python:3.11-slim
- Install system deps: libpq-dev, libmagic1
...
- Serve stage: nginx:alpine, copy build to /usr/share/nginx/html

Environment variables (never in code — always in .env.prod):
DJANGO_SECRET_KEY, DATABASE_URL, REDIS_URL, 
ENCRYPTION_KEY (32-byte hex), BLIND_INDEX_KEY (32-byte hex),
AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY, AWS_S3_BUCKET_NAME, AWS_S3_REGION,
RAZORPAY_KEY_ID, RAZORPAY_KEY_SECRET,
WHATSAPP_API_TOKEN, SENDGRID_API_KEY, MSG91_AUTH_KEY

GitHub Actions CI/CD (.github/workflows/deploy.yml):
- On push to main: run backend tests (pytest), run frontend tests (vitest), build Docker images, push to ECR, deploy to ECS/EC2
...
- S3 bucket: Block all public access, SSE-S3 enabled, versioning enabled
- Redis: AUTH password set, bind to private IP only
- Django CONN_MAX_AGE=60 for connection pooling
```
...
|--------|----------|-------------|---------------|
| 1 | 2 weeks | Django bootstrap, encryption utils, GST utils, S3 service, RBAC, React bootstrap | Encrypt/decrypt tests pass. Tenant isolation test passes. |
| 2 | 2 weeks | Super Admin: Tenant Mgmt, Package/Plan/Subscription APIs + UI | SA can create tenant, assign subscription, see usage meters |
| 3 | 2 weeks | Services: Customer + Vehicle with DPDP consent, Vehicle alerts | Customer created with consent. PII encrypted in DB. |
| 4 | 2 weeks | Services Master (categories + items), Job Card create + Kanban | Job card created with GST calculation. State machine enforced. |
| 5 | 2 weeks | Invoice generation, PDF, S3 upload, WhatsApp delivery | Invoice generated. PDF in S3. WhatsApp send working. |
| 6 | 2 weeks | Portfolio: Inventory Vehicle (all types), Image upload to S3 | Vehicle listed with images. Presigned URLs returned. |
| 7 | 2 weeks | Public Portfolio page, Test Drive booking, OTP verification | Public URL live. Test drive booking creates lead. |
| 8 | 2 weeks | Analytics dashboards (services + portfolio), Subscription History UI | Charts render with real data. |
| 9 | 2 weeks | DPDP compliance endpoints, Rate limiting, Security headers, MFA | Erasure request works. Rate limit returns 429. |
| 10 | 2 weeks | Full test suite (80% coverage), Load testing, Audit log review | All critical tests pass. No PII in logs verified. |
| 11 | 2 weeks | Docker prod setup, CI/CD pipeline, Production deploy, UAT | UAT sign-off. Zero PII in plaintext in DB. |

---

*VeHubPro Cursor Development Plan v1.0.0 — Stack: React.js + Django + PostgreSQL + AWS S3 — Compliance: DPDP 2023 · GDPR · ISO 27001*
